  ...
  return (
    <div className={styles.mainSearch}>
      ...
      <div className={styles.filters}>
        <Select
          value={animalType.find((item) => item.value === type)}
          options={animalType}
          styles={inputStyles}
          placeholder='Тип'
          onChange={({ value }) =>
            updateSearchParams({ gender, status, city, date, type: value })
          }
        />
        <Select
          value={animalStatus.find((item) => item.value === status)}
          options={animalStatus}
          styles={inputStyles}
          placeholder='Статус'
          onChange={({ value }) =>
            updateSearchParams({ type, gender, city, date, status: value })
          }
        />
        <Select
          value={animalGender.find((item) => item.value === gender)}
          options={animalGender}
          styles={inputStyles}
          placeholder='Стать'
          onChange={({ value }) =>
            updateSearchParams({ type, status, city, date, gender: value })
          }
        />
        <input
          name='city'
        ...
        <button
          onClick={() =>
            Router.push(
              `/search?type=${type || 'all'}&gender=${gender || 'all'}&status=${
                status || 'all'
              }&city=${city || 'all'}&date=${date || 'all'}&page=1`
            )
          }
      ...
      <div className={styles.actions}>
        <Button fullWidth link='/create'>
          Створити оголошення
        </Button>
        <Button fullWidth>Придбати нашийник</Button>
      </div>
    </div>
  );
};
